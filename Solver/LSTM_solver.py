...
class Solver:
    ...
    def train(self):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            inputs = inputs.unsqueeze(1)  # inputs의 형태를 [batch_size, 1, 7]로 변경
            inputs.requires_grad = True  # gradient 계산 가능
            labels = labels.squeeze(1)
            self.optimizer.zero_grad()

            outputs = self.net(inputs).cuda()  # 288*18
            # Feed-forward
    ...
    def val(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.val_loader):
                # Load batch data
                inputs, labels = data[0].cuda(), data[1].cuda() #shape torch.size([32,7]),  torch.size([32,1])
                inputs = inputs.unsqueeze(1)  # inputs의 형태를 [batch_size, 1, features]로 변경
                labels = labels.squeeze(1).long()
                # Feed-forward
                outputs = self.net(inputs) #outputs.shape torch.size([32,3])
                loss = self.criterion(outputs, labels) #torch.Size([])
    ...
    def experiment(self):
        ...
        for name, param in self.net.named_parameters():
            if name != "hidden2out.weight" and name != "hidden2out.bias":
